...
class Chart extends Component {
	...
	mapData(mode) {
		...
		if (mode === "y") {
			...
			filtered = filtered.filter(entry => {
				return (
					entry.date >= this.state.first.toISOString() &&
					entry.date < this.state.last.toISOString()
				);
			});
		} else if (mode === "w") {
			let today = moment();
			...
			filtered = filtered.filter(entry => {
				return (
					entry.date >= l7d.toISOString() && entry.date < today.toISOString()
				);
			});
		}
		...
		this.setState({ total_expenditure: total });
	...
	render() {
		...
		return (
			<div>
				<br />
				<Line width={900} height={300} data={payload} />
				<br />
				<div
					className="row justify-content-center"
					style={{ margin: "0px !important" }}
				>
					<button className="btn-sm btn active" onClick={this.mapAll}>
						All
          </button>
					<button className="btn-sm btn" onClick={this.mapYear}>
						This year
          </button>
					<button className="btn-sm btn" onClick={this.mapMonth}>
						This month
          </button>
					<button className="btn-sm btn" onClick={this.mapWeek}>
						Last 7 days
          </button>
				</div>
				<br />
				<div style={{ textAlign: "center" }}>
					<p>
						<b>
							Total expenditure for timescale: ${this.state.total_expenditure}
						</b>
					</p>
				</div>
			</div>
